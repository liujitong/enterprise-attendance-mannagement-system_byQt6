...
#include "ui_resi_superadmin.h"
#include "publicdb.h"
#include "QRegularExpression"
#include "QRegularExpressionMatch"
#include <QMessageBox>
resi_superadmin::resi_superadmin(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::resi_superadmin)
{
    ui->setupUi(this);
}

resi_superadmin::~resi_superadmin()
{
    delete ui;
}

void resi_superadmin::on_pushButton_2_clicked()
{
    QSqlQuery qb(db);
    QString name;
    QString pss;
    ...
    else if(!(rpss.match(pss)).hasMatch())
    {
        ...
        if(qb.exec(QString("INSERT INTO account VALUES('%1','%2','0');").arg(name).arg(pss)))
        {
            //提示窗格
